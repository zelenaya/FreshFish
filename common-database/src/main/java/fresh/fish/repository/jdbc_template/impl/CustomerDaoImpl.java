...
//        customer.setDate_modificate(resultSet.getTimestamp(DATE_MOD));
    ...
    private CustomerService getCustomerServiceRowMapper(ResultSet rs, int i) throws SQLException {
        CustomerService customerService = new CustomerService(getCustomerRowMapper(rs, i));
        customerService.setUser(new User(rs.getLong("user_id"), rs.getString("login"), "********"));
    ...
    @Override
    public List<CustomerService> getFullInfo() {
        String query = "SELECT * FROM customers c LEFT JOIN users u ON c.cust_user_id = u.user_id";
    ...
    @Override
    public CustomerService getFullInfo(Long id) {
        String query = "SELECT * FROM customers c LEFT JOIN users u ON c.cust_user_id = u.user_id WHERE c.cust_id = ?";
        CustomerService customerService = jdbcTemplate.queryForObject(query, new Object[]{id}, this::getCustomerServiceRowMapper);
        customerService.setRoles(roleDao.getRolesByUserId(customerService.getUser().getUserId()));
        customerService.setOrders(orderDao.findByCustomerId(id));
